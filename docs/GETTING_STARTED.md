...
1.  **Open an Agda file:** Open one of the `.agda` files from this repository in your editor.
2.  **Load the file:** In your editor, use the "load" command. In both Emacs and VS Code, the standard keybinding for this is `C-c C-l` (Ctrl+C, then Ctrl+L).
...
We recommend reading the files in the following order to understand the project's structure:
